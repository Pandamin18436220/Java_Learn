 ...
 * 不必每次获取对象时都要进行同步，只有第一次才同步，创建了以后就没有必要了
 */
public class SingletonDemon03 {
    //类的初始化，不初始化这个对象（延时加载的优势）,加载类的时候是一个天然的线程安全
    private static SingletonDemon03 instance;
    private SingletonDemon03(){

    }
    //方法同步，调用效率低
    public static SingletonDemon03 getInstance(){
        if(null==instance){
            SingletonDemon03 sc;
            synchronized (SingletonDemon03.class){
                sc = instance;
                if(sc==null){
                    synchronized (SingletonDemon01.class){
                        if(sc==null){
                            sc = new SingletonDemon03();
                        }
                    }
                    instance = sc;
                }
